...
function InfoButton () {
    ...
    return (
        <div>
            ...
            <div className="mt-2">{info?.map((info:any) =>{return <ul key={info.id} className="text-red-500 bg-yellow-500"><button onClick={()=> handleRedirect(`/info-page/${info.id}`)} className="mx-auto mt-2 items-center p-4 min-w-40 border-red-500 rounded-md border-4 bg-orange-200 hover:shadow-xl hover:bg-orange-600 hover:text-white">{info.name}</button></ul>})}</div>
        </div>
    )
}
